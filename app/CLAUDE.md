# CLAUDE.md — talkverselab/talkverse-ar
...
- 이름은 **`talkverselab/talkverse-<언어코드>`** 로 통일(2026-09-10). 옛 이름(`ar-universe` 등)은 리다이렉트되지만
  remote는 새 이름으로 바꿔 둘 것: `git remote set-url origin https://github.com/talkverselab/talkverse-ar.git`
...
- **이전 코드·자산은 `talkverse/_legacy/ar/`** 에 그대로 있다(리포 밖). git 이력에도 남아 있다.
- 옮긴 콘텐츠: 이집트 아랍어 회화 7편(77턴, 장면별) · 고빈도 단어 1,000(단어) + 5,000(빈도) · 회화 표현 포인트 1강. 플레이버 20개 구조는 걷어냈고 단일 앱(`com.talkverse.ar`)이다.
